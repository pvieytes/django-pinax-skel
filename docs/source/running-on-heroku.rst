...
``django-pinax-skel`` ships with a production ready Heroku configuration module, and
...
upgrade them at any time in the future). ``django-pinax-skel`` already supports all
...
(``django-pinax-skel`` requires this). To do this, run::
...
``django-pinax-skel`` requires several environment variables be set. To set these
...
at the root of your ``django-pinax-skel`` project defines our three service types:
